...
class Wc_Bank_Transfer_Select_Public {
	 ...
	 * @since    1.0
	 */
	function dc_save_bacs_option_to_order_meta( $order, $data ) {

		// todo check if I select a bank (not empty) and then switch payment method - if it saves or not
		if ( ! empty( $_POST['dc_bacs_option'] ) ) {

			$order->update_meta_data( 'dc_bacs_option', wc_clean( $_POST['dc_bacs_option'] ) );
		}
	}
